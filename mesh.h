...
struct VoxelMesh {
    ...
    int gen_vertices(const dim3 &cube, double level, std::vector<point> &pts) {
        double v[8];
        point newpts[4];
        int cnt[4] = {0, 0, 0, 0};
        for (int i = 0; i < 4; i++)
        ...
        for (int i = 0; i < 12; i++) {
            ...
            bool c1 = v1 > level;
            bool c2 = v2 > level;
            update_edge(cube, e, i, c1, c2, patchid);
            if (c1 == c2) {
                assert(patchid == 3);
                continue;
            }
